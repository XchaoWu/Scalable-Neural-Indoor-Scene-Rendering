...
inline __host__ __device__ float3 operator/(float s, float3 a)
{
    // float inv = 1.0f / s;
    // return a * inv;
    float3 inv_a = make_float3(1.0f/a.x, 1.0f/a.y, 1.0f/a.z);
    return s * inv_a;
}
inline __host__ __device__ void operator/=(float3 &a, float s)
{
...
inline __device__ __host__ float3 clamp(float3 v, float a, float3 b)
{
    return make_float3(clamp(v.x, a, b.x), clamp(v.y, a, b.y), clamp(v.z, a, b.z));
}
...
inline __device__ __host__ float3 max_dim(const float3* a, int num)
{
    float3 out = a[0];
    for (int i=1; i<num; i++)
    {
...
inline __device__ __host__ float3 min_dim(const float3* a, int num)
{
    float3 out = a[0];
    for (int i=1; i<num; i++)
    {
...
inline __device__ __host__ float3 ceilf(float3 a)
{
    return make_float3(ceil(a.x), ceil(a.y), ceil(a.z));
}

inline __device__ __host__ int signf(float a)
{
    return a >= 0? 1:-1;
}

inline __device__ __host__ int3 signf(float3 a)
{
    return make_int3(signf(a.x), signf(a.y), signf(a.z));
}


inline __device__ __host__ float safe_divide(float a, float b){
    return b!=0? a/b:100000000.0f;
}

inline __device__ __host__ float3 safe_divide(float3 a, float3 b)
{
    return make_float3(safe_divide(a.x,b.x), 
                       safe_divide(a.y,b.y),
                       safe_divide(a.z,b.z));
}


inline __device__ __host__ float norm(float3 v)
{
    return sqrtf(dot(v, v));
}

inline __device__ __host__ float3 expf(float3 v)
{
    return make_float3(expf(v.x), expf(v.y), expf(v.z));
}

inline __device__ __host__ float min_value(float3 v)
{
